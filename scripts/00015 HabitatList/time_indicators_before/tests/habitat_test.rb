...
class HabitatTest < Minitest::Test
  ...
  def test_environment_grouping_preserves_conditions_tools_systems_and_status
    ...
    assert_equal %i[a masked night lake surf rod good_rod], display.groups.map(&:key)
    assert_equal %i[active shadowed inactive active active active active], display.groups.map(&:status)
