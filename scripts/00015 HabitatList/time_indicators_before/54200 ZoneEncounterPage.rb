...
module UI
  module Dex
    # Presentation only: terrain tags and battle formats are technical variants.
    # Conditions, tools and priority status remain separate, as in the catalogue.
    module ZoneEncounterEnvironments
      def self.build(snapshot)
        groups = snapshot.groups.group_by do |view|
          ...
          conditions = data.custom_conditions.map do |condition|
            [condition.type, condition.value, condition.relation_with_previous_condition]
          end
          [data.system_tag, data.tool, conditions, view.status]
        end.values.map do |members|
          view = members.first.dup
          data = data_group(view.key)
      ...
      def initialize(viewport)
        ...
        @cells = Array.new(PAGE_SIZE) do |index|
          ...
          {sprite: sprite, fallback: fallback, badge: badge,
           center_x: x + CELL_WIDTH / 2, center_y: y + CELL_HEIGHT / 2}
        end
      end
...
module GamePlay
  class ZoneEncounters < BaseCleanUpdate::FrameBalanced
    ...
    def refresh_catalog(force: false)
      ...
      return if !force && snapshot == @catalog_snapshot
