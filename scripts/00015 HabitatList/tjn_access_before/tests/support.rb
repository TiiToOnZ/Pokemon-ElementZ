...
# Actual TJN schedules and lookup, with no native tone/window initialization.
tjn_source = File.read(File.join(ENGINE, '4_Systems_003_Map_Engine.rb'), encoding: 'UTF-8')
module Yuki::TJN; end
Yuki::TJN.module_eval(tjn_source[/^    TIME_SETS = .*$/])
Yuki::TJN.module_eval(tjn_source[/^    TIME = .*$/])
Yuki::TJN.module_eval(tjn_source[/^      def current_time_set\n.*?^      end/m])
Yuki::TJN.send(:module_function, :current_time_set)
