...
class HabitatTest < Minitest::Test
  ...
  def test_unknown_seen_caught_and_ensemble_indicators_and_reused_cells
    group(:a, [encounter(:rattata, 0), encounter(:rattata, 1)])
    zone([:a])
    value = scene
    view = value.instance_variable_get(:@page)
    cells = view.instance_variable_get(:@cells)
    assert view.instance_variable_get(:@legend_icons).all?(&:visible)
    assert cells.flat_map { |c| c[:times] }.none?(&:visible)
    RPG::Cache.time_reads.clear
    value.send(:action_a)
    assert_empty RPG::Cache.time_reads # Unknown cells do not even load time art.
    assert cells.flat_map { |c| c[:times] }.none?(&:visible)
    refute cells[0][:badge].visible
    ...
    $pokedex.mark_seen(:rattata, 0, forced: true)
    value.send(:refresh_catalog, force: true)
    assert cells[0][:times].all?(&:visible)
    refute cells[0][:badge].visible
    assert cells[1][:times].none?(&:visible) # The unseen form remains hidden.
    $pokedex.mark_captured(:rattata, 0)
    value.send(:refresh_catalog, force: true)
    assert cells[0][:badge].visible
    assert_equal %w[daytime nighttime], cells[0][:times].map { |i| File.basename(i.bitmap.asset, '.png') }
  ...
  def test_legend_uses_native_tjn_lookup_including_tone_only_fallback_and_live_changes
    ...
    value = scene
    view = value.instance_variable_get(:@page)
    assert_equal ['Matin/Jour 07:00–19:00', 'Soir/Nuit 19:00–07:00'], view.instance_variable_get(:@legend_texts).map(&:text)
    ...
    assert_equal ['Matin/Jour 10:00–16:00', 'Soir/Nuit 16:00–10:00'], view.instance_variable_get(:@legend_texts).map(&:text)
  end
