...
class HeadlessDrawable
  attr_accessor :visible, :text, :multiline_text, :bitmap, :nb_x, :y, :x, :bold, :data,
                ...
                :outline_thickness, :align, :font_id
  def initialize(*)
    @visible = true
  end
  ...
  module Cache
    class << self
      attr_accessor :fail_icon, :icon_reads, :fail_front, :front_reads, :question_reads,
                    :fail_question, :front_dimensions, :raise_front
      ...
      def poke_front_exist?(_filename)
        !fail_front
      end
      def poke_front(filename)
        raise 'Corrupt texture' if raise_front
        self.front_reads ||= []
        front_reads << filename
        width, height = (front_dimensions || {}).fetch(filename, [96, 96])
        OpenStruct.new(width: width, height: height, asset: "graphics/pokedex/pokefront/#{filename}.png")
      end
      ...
      def pokedex(filename)
        self.question_reads ||= []
        question_reads << filename
        OpenStruct.new(width: 96, height: 96, asset: "graphics/pokedex/#{filename}.png")
      end
    end
  end
end
