...
class HabitatTest < Minitest::Test
  ...
  def test_hidden_ui_does_not_resolve_names_forms_or_sprites
    ...
    assert_equal 'graphics/pokedex/000.png', cell[:sprite].bitmap.asset
  ...
  def test_more_than_twelve_entries_and_empty_navigation
    ...
    value.send(:change_page, 1)
    assert_equal 16, value.instance_variable_get(:@offset)
    zone([])
    value.send(:refresh_catalog, force: true)
  ...
  def test_grid_centers_varied_dimensions_without_distortion_clipping_or_button_overlap
    ...
    RPG::Cache.front_dimensions = {'0019' => [24, 32], '0025' => [256, 48], '0678_01' => [48, 256]}
    cells = scene.instance_variable_get(:@page).instance_variable_get(:@cells)
    cells.first(3).each_with_index do |cell, index|
      sprite = cell[:sprite]
      assert_equal sprite.zoom_x, sprite.zoom_y
      assert_equal 46 + index * 76, sprite.x
      assert_equal 87, sprite.y
      width = sprite.bitmap.width * sprite.zoom_x
      height = sprite.bitmap.height * sprite.zoom_y
      assert_operator width, :<=, 68
      assert_operator height, :<=, 62
      assert_operator sprite.x - width / 2, :>=, 12 + index * 76
      assert_operator sprite.y - height / 2, :>=, 56
      assert_operator sprite.y + height / 2, :<=, 118
    end
  ...
  def test_overview_is_sprite_only_and_pagination_clears_unused_cells
    ...
    assert_equal '20 Pokémon', view.instance_variable_get(:@counts).text
    assert_equal 'Page 1/3', view.instance_variable_get(:@pagination).text
    assert_equal '', view.instance_variable_get(:@availability).text
    value.send(:change_page, 2)
    assert_equal 'Page 3/3', view.instance_variable_get(:@pagination).text
    cells = view.instance_variable_get(:@cells)
    assert_equal 4, cells.count { |cell| cell[:sprite].visible }
    refute $habitat_text_reads.any? { |file, _id| [0, 67].include?(file) }
  end
  ...
  def test_milieus_distinguish_time_terrain_tool_and_priority
    ...
    view = value.instance_variable_get(:@page)
    value.send(:action_a)
    assert_match(/Matin/, view.instance_variable_get(:@context).text)
    assert_match(/Inactif/, view.instance_variable_get(:@availability).text)
    value.send(:action_a)
    assert_match(/Nuit/, view.instance_variable_get(:@context).text)
    value.send(:action_a)
    assert_match(/terrain 2/, view.instance_variable_get(:@context).text)
    value.send(:action_a)
    assert_match(/Pêche.*Canne.*Lac/, view.instance_variable_get(:@context).text)
    value.send(:action_a)
    assert_match(/Masqué.*prioritaire/, view.instance_variable_get(:@availability).text)
  end
